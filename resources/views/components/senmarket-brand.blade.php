...
@php
    $logoSize = $small ? '48px' : '120px';
    $titleSize = $small ? '1.35rem' : '2rem';
    $subtitleSize = $small ? '0.82rem' : '1.1rem';
    $isDark = $theme === 'dark';
...
<div style="{{ $centered ? 'text-align:center;' : '' }}">
    <a href="{{ $href }}" style="display:inline-flex; align-items:center; gap:14px; text-decoration:none; color:{{ $senColor }};">
        <img src="{{ asset('images/ndarmarket-logo.png') }}" alt="Logo NdarMarket" style="width:{{ $logoSize }}; height:{{ $logoSize }}; object-fit:contain; border-radius:14px; background:{{ $isDark ? 'rgba(255,255,255,.06)' : 'transparent' }};">
        @if(!$hideText)
        <span>
            <span style="display:block; font-size:{{ $titleSize }}; font-weight:800; line-height:1.1; letter-spacing:-0.02em;">
                <span style="color:{{ $senColor }};">Ndar</span><span style="color:{{ $marketColor }};">Market</span>
            </span>
            @if($showSlogan)
